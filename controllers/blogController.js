...
const {isValid, isValidRequestBody, isValidObjectId,isValidTag} = checkValid
...
const deleteBlogById = async function (req, res) {
  try {
       const filterQuery = { isDeleted: true}
        ...
        let finddelete=await blogModel.findOne({_id:blogId,isDeleted:true})
        if(finddelete)return res.status(404).send({ status: false, message: "No matching blog found" }) 
